...
class NavBarComponent extends React.Component {
  ...
  render() {
    const drawer = (
      <StyledDrawer>
        <StyledAvatar alt="logo" src="https://d1muf25xaso8hp.cloudfront.net/https%3A%2F%2Fs3.amazonaws.com%2Fappforest_uf%2Ff1521686436689x671059151878580500%2FArtboard%25203.png?w=256&h=78&auto=compress&fit=crop"/>
        {this.props.path === '/profile' ?
          ...
          <div>
            {navInfor.navItems.map((items,i) => {
              let panel = 'panel' + i;
              return (  <StyledExpansionPanel expanded={this.state.expanded === panel} onChange={this.handleChange(panel)} key={i}>
                          <ExpansionPanelSummary expandIcon={<ExpandMoreIcon />} onClick={() => {this.onItemChange(items)}}>
                          ...
                          <StyledExpansionPanelDetails>
                            {items.subItems.map((subItem, idx) => {
                              return (
                                <NavSubItemTitle button onClick={() => {this.onSubItemChange(subItem)}} key={idx}>
                                {subItem.title}
                                </NavSubItemTitle>
                              )
                            })}
            ...
            <LogoutLink>
              <LogoutIcon className="ion-ios-redo inner-element bubble-element" onClick={() => {this.onLogOut()}}>                           
