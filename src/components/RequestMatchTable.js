          ...
          <StyledTable>
              ...
              <TableRow>
                {
                  RequestTableInfo.header.map((header,i) => {
                    return (
                      header === 'OLDEST BRIEF' ?
                      <StyledTableCellOldBrief align="left" key={i}>{header}</StyledTableCellOldBrief> : <StyledTableHeaderCell key={i}>{header}</StyledTableHeaderCell>
                    )
                  })}
              </TableRow>
            </StyledHeadRow>
            <TableBody>
              {this.state.Info.map(row => (
                <StyledTableRow key={row.id} onClick={() => {this.onRowClick(row.id)}}>
                  <StyledID align="left">
                    {row.id}
                  </StyledID>
                  ...
                  <TableCell align="center">{row.nbCallsScheduledForOldestBrief}</TableCell>
                </StyledTableRow>
              ))}
            </TableBody>
          </StyledTable>
...
export default connect(mapStateToProps, mapDispatchToProps)(RquestMatchTable);